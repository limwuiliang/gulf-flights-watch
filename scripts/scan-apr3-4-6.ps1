...
# Query Apr 3, 4, 6 (skip Apr 5, keep cloned data)
$datesToScan = @(
  [System.DateTime]::Parse("2026-04-03"),
  [System.DateTime]::Parse("2026-04-04"),
  [System.DateTime]::Parse("2026-04-06")
)
...
# Remove Apr 3, 4, 6 from existing flights (replace with real API data)
foreach ($airline in $json.airlines) {
  $airline.flights = @($airline.flights | Where-Object { $_.date -notmatch "2026-04-03|2026-04-04|2026-04-06" })
}
...
# Update flightVolumeHistory for Apr 3, 4, 6
foreach ($dateStr in @("2026-04-03", "2026-04-04", "2026-04-06")) {
  $entry = $json.flightVolumeHistory | Where-Object { $_.date -eq $dateStr }
  if (-not $entry) {
    $entry = @{ date = $dateStr; emirates_scheduled = 0; emirates_departed = 0; qatar_scheduled = 0; qatar_departed = 0; etihad_scheduled = 0; etihad_departed = 0 }
    $json.flightVolumeHistory += $entry
  }
...
git commit -m "scan: Apr 3, 4, 6 real API data (quota reset)"
